...
int main(object caller, object room, string str) {
    ...
    switch(act) {
        case 0..17:
            ...
            tell_room(here, capitalize(tp->name()) + " runs around looking like a complete loon.\n", ({ tp }));
            break;
        case 18..29: {
                ...
                if(arr_sz) {
                    ...
                    if(here->valid_exit(direction)) {
                        ...
                        if(tp->allow_move(there)) {
                            tell_room(here, capitalize(tp->name()) + " leaves through the " + direction + " exit.\n", ({ tp }));
                            write("You move to " + there->query_short() + ".\n\n");
                            tell_room(there, capitalize(tp->name()) + " has entered the room.\n", ({ tp }));
                            tp->move(there);
                        }
                    }
            }
        ...
        case 30..36:
          if(random(100) < 75) {
                ...
                tell_room(here, capitalize(tp->name()) + " fades out and then fades back into existence.\n", ({ tp }));
            } else {
