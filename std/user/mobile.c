...
void exit_world() {
    ...
    if(environment())
        say((string)capitalize(query_name())+ " leaves " + mud_name() + ".\n");
...
void net_dead() {
    ...
    set("last_login", time());
    save_user();
    if(environment()) tell_room(environment(), capitalize(query_name()) + " has gone link-dead.\n");
...
void reconnect() {
    ...
    if(environment()) tell_room(environment(), capitalize(query_name()) + " has reconnected.\n", this_player());
...
void heart_beat() {
    if(!interactive(this_object())) {
        if((time() - query("last_login")) > 3600) {
            if(environment())
                tell_room(environment(), capitalize(query_name()) + " fades out of existance.\n");
